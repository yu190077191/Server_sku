...
function deleteItem(tableName, id, rowId, callback) {
    if (confirm(getConfirmDeleteMessage())) {
        var url = getRootUrl() + "Common/Delete";
...
function deleteGroupedItems(tableName, id) {
    if (confirm(getConfirmDeleteMessage())) {
        $.post(getRootUrl() + "Common/Delete", { tableName: tableName, id: id }, function (json) {
            if (json.Success) {
                $("#tr" + id).css({ "display": "none" });
...
function restoreItem(tableName, id, rowId) {
    //if (confirm(getConfirmDeleteMessage())) {
        $.post(getRootUrl() + "Common/Restore", { tableName: tableName, id: id }, function (json) {
            if (json.Success) {
                if (rowId == null) {
                    $("#tr" + id).css({ "display": "none" });
                }
...
function getCheckboxList(tag) {
    ...
    for (var i = 0; i < checkboxes.length; i++) {
        ...
        if (id != null && id.toLowerCase() != "selectall" && id.indexOf(tag) == 0) {
            arr.push(id.replace(tag, ""));
        }
    }
...
var selectedcheckbox_Array = new Array();

function saveSelectStatus() {
    var checkboxes = $(":checkbox");
    var arr = new Array();
    for (var i = 0; i < checkboxes.length; i++) {
    ...
    selectedcheckbox_Array = arr;
}

function restoreSelectStatus() {
    $(selectedcheckbox_Array).each(function () {
        var chk = document.getElementById(this);
        chk.checked = true;
    });
}
...
function selectAll(obj) {
    ...
    if (checkboxArray != null && checkboxArray.length > 0) {
        for (var i = 0; i < checkboxArray.length; i++) {
            var obj = document.getElementById("checkbox" + checkboxArray[i]);
            if (obj == null) continue;
            obj.checked = selectStatus;
        }
    }
}

function getSelectedIds(tag) {
    if (tag == null) {
        tag = "checkbox";
    }
...
function deleteAll(tableName) {
    ...
    if (ids == "") alert("未选择任何项！");
        ...
        $.post(getRootUrl() + "Finance/DeleteAll", { tableName: tableName, ids: ids }, function (json) {
            if (json.Success) {
                var array = new Array();
                array = ids.split(',');
                for (var i = 0; i < array.length; i++) {
...
function formatMoney(obj) {
    ...
    function getValue(elementId, isRequired, dataType) {
        ...
        if ($(elementId)[0].type != null && $(elementId)[0].type.toLowerCase() == "textarea" && $(elementId).attr("title") != null) {
    ...
    $(document).ready(function () {
        if (document.getElementById("jsonModelJs") != null) {
            ...
            for (var elementId in jsonModel) {
                ...
                if (obj != null) {
                    ...
                    if (obj.tagName != null && ",td,span,label,".indexOf(obj.tagName.toLowerCase()) >= 0) {
        ...
        waterMarkTextareas();
        if (document.getElementById("LoadDataReadyMarker") != null) {
            onLoadDataReady();
        }
    });
    ...
    function setLanguage(languageEnglishName) {
        $.post(getRootUrl() + "Home/SetLanguage", { languageEnglishName: languageEnglishName }, function (response) {
