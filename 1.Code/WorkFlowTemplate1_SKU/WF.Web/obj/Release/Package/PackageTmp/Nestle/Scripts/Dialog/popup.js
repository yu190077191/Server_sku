var CONTEXTPATH = "/SKURegistration/Nestle/Scripts/Dialog/";
...
function popup(html, divId, followCursor) {
    ...
    var sh = Math.max(doc.documentElement.scrollTop, doc.body.scrollTop); //考虑滚动的情况
     ...
     if (div == null) {
         var arr = [];
         arr.push("<table style='-moz-user-select:none;' border='0' cellpadding='0' cellspacing='0' class='popup'>");
         ...
         arr.push("      <div class='closePopButton' title='close' style=\"position: relative;cursor:pointer; float:right; margin:5px 0 0; _margin:4px 0 0;height:17px; width:28px; background-image:url(" + CONTEXTPATH + "dialog_closebtn.gif)\" onMouseOver=\"this.style.backgroundImage='url(" + CONTEXTPATH + "dialog_closebtn_over.gif)'\" onMouseOut=\"this.style.backgroundImage='url(" + CONTEXTPATH + "dialog_closebtn.gif)'\" onClick=\"closeDialogDiv();\"></div></td>");
