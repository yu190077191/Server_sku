...
namespace WF.BusinessRule
{
    public class AttachmentRule : System.Web.UI.Page
    {
        ...
        {
            return AttachmentDao.SaveData(storedProcedureName, UniqueId, id);
        }
        ...
        {
            return AttachmentDao.Upload(fileFullPath, tempTableName, storedProcedureName, UniqueId, maxColumnCount, addUploadedTime);
        }

        public void Download(Guid attachmentId)
        {
            Attachment model = CommonRule.GetModel<Attachment>(guid : attachmentId);
            if (model != null)
            {
                HttpResponse resp = HttpContext.Current.Response;
                resp.ContentEncoding = System.Text.Encoding.GetEncoding("GB2312");
                resp.AppendHeader("Content-disposition", "attachment;filename=" + HttpUtility.UrlPathEncode(model.Description) + Path.GetExtension(model.FilePath));
                string filePath = GetFullPath(model.FilePath);
                resp.AddHeader("Content-type", MimeType(filePath));
                resp.AddHeader("Accept-Ranges", "bytes");
                resp.AddHeader("Content-Length", new System.IO.FileInfo(filePath).Length.ToString());
                resp.WriteFile(filePath);
                resp.End();
            }
        }

        public static bool Delete(Guid attachmentId)
        {
            Attachment model = CommonRule.GetModel<Attachment>(guid : attachmentId);
            //if (model != null && (model.CreatedBy == Operation.OperationBy || EmployeeRoleRule.IsAdmin()))
            //{
                if (CommonRule.UpdateRecordStatus("Attachment", attachmentId.ToString(), 2) > 0)
                {
                    //File.Delete(GetFullPath(model.FilePath));
                    return true;
                }
            //}
        ...
        {
            return AttachmentDao.InsertAttachment(model);
        }

        public static int UpdateAttachment(Attachment model)
        {
            return AttachmentDao.UpdateAttachment(model);
        }

        public static List<Attachment> GetAttachment(string procedureName, SqlParameter[] sqlParamters)
        {
            return AttachmentDao.GetAttachment(procedureName, sqlParamters);
        }

        public static List<Attachment> GetAttachment(int requestVersionId, string typeCode)
        {
            return AttachmentDao.GetAttachment("GetAttachment", new[] { new SqlParameter("@requestVersionId", requestVersionId), new SqlParameter("@typeCode", typeCode) });
        }

        public static QueryResult<Attachment> GetAttachmentResult(string procedureName, SqlParameter[] sqlParamters)
        {
            return AttachmentDao.GetAttachmentResult(procedureName, sqlParamters);
        }
