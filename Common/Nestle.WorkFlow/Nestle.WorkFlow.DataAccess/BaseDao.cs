...
namespace Nestle.WorkFlow.DataAccess
{
    ...
    {
        public static readonly string ConnectionString = ConfigurationManager.ConnectionStrings["Nestle.WorkFlow.Database2"].ConnectionString;

        public static readonly int PageSize = Convert.ToInt32(ConfigurationManager.AppSettings["PageSize"], Constants.CurrentCulture);
        ...
        public static int ExecuteNonQuery2(string commandText, SqlParameter[] sqlParameters, CommandType commandType = CommandType.StoredProcedure)
        {
            int result = -1;
            var paramList = new List<SqlParameter>();
            AddSiteSettingAndOpertionBy(ref paramList);
